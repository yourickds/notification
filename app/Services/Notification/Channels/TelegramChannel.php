...
final class TelegramChannel implements ChannelInterface
{
    public function send(Notification $notification): bool
    {
        ...
        if (rand(1, 100) <= 30) {
            throw new RuntimeException("Telegram 5xx: temporary failure for notification #{$notification->id}");
        }

        // 15% шанс на фатальную ошибку (4xx) → без retry, сразу ERROR
        if (rand(1, 100) <= 15) {
            throw new FatalChannelException(
                "Telegram 4xx: invalid chat_id for user #{$notification->user_id} (notification #{$notification->id})"
            );
        }
